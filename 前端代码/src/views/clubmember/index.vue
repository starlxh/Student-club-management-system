...
export default {
  ...
  methods: {
    getList() {
      ...
      request
        .get(this.baseUrl + 'queryClubMemberList', { params: this.listQuery })
        .then((res) => {
          this.list = res.data
    ...
    createData() {
      this.$refs['createForm'].validate((valid) => {
        if (valid) {
          ...
          request
            .post(
              this.baseUrl + 'addClubMember',
              JSON.parse(JSON.stringify(this.temp, ['userId', 'joinTime', 'clubId']))
            )
            .then((res) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.temp.joinTime = new Date(this.temp.joinTime)
            .toLocaleString()
            .replaceAll('/', '-')
          request
            .post(
              this.baseUrl + 'editClubMember',
              JSON.parse(
                JSON.stringify(this.temp, ['clubMemberId', 'joinTime', 'clubId'])
              )
            )
            .then((res) => {
    ...
    handleDelete(row, index) {
      request
        .delete(this.baseUrl + 'deleteById?clubMemberId=' + row.clubMemberId)
        .then((res) => {
          if (res.code === 20000) {
            this.$notify({
              title: '成功',
