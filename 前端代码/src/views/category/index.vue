...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        categoryName: [
          { required: true, message: '请输入社团类型名称', trigger: 'blur' }
        ]
      },
  ...
  methods: {
    getList() {
      ...
      request
        .get(this.baseUrl + 'queryCategoryList', {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          request.post(this.baseUrl + 'addCategory', this.temp).then((res) => {
            if (res.code === 20000) {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          request.post(this.baseUrl + 'editCategory', this.temp).then((res) => {
            this.dialogFormVisible = false
            if (res.code === 20000) {
              this.$notify({
                title: '成功',
    ...
    handleDelete(row, index) {
      request
        .delete(this.baseUrl + 'deleteById?categoryId=' + row.categoryId)
        .then((res) => {
          if (res.code === 20000) {
            this.$notify({
              title: '成功',
