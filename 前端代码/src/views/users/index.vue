<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.type" placeholder="权限类型" clearable class="filter-item" style="width: 150px">
...
export default {
  ...
  methods: {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          request.post(this.baseUrl + 'editUser', JSON.parse(JSON.stringify(this.temp, ['userId', 'userName', 'password', 'realName', 'sex', 'tel', 'email', 'qq', 'wx', 'createTime']))).then(
            res => {
    ...
    handleDelete(row, index) {
      request.delete(this.baseUrl + 'deleteById?userId=' + row.userId).then(
        res => {
          if (res.code === 20000) {
            this.$notify({
              title: '成功',
